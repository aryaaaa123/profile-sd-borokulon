...
@section('content')
<style>
  .thumb-4x3{ position:relative; width:100%; padding-top:75%; overflow:hidden; background:#f2f5f9; }
  .thumb-4x3 > img{ position:absolute; inset:0; width:100%; height:100%; object-fit:cover; transition:transform .35s ease; }
  .card-zoom:hover .thumb-4x3 > img{ transform:scale(1.06); }
...
<section class="container py-5">
  <div class="d-flex justify-content-between align-items-end flex-wrap gap-2">
    ...
    {{-- Optional: total item --}}
    @isset($items)
      <span class="text-muted small">{{ method_exists($items,'total') ? $items->total() : $items->count() }} item</span>
    @endisset
  </div>
    ...
    <div class="row row-cols-1 row-cols-md-2 row-cols-lg-3 g-4 mt-3 justify-content-center">
      @foreach($items as $s)
        @php
          ...
          $img = $s->image_url
              ?? (isset($s->image_path) ? asset('storage/'.$s->image_path) : asset('assets/images/placeholder-sarpras.jpg'));
        @endphp
